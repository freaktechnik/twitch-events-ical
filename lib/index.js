...
const Calendar = require("./calendar"),
    ...
    cache = require("./cache"),
    LIMIT = 100,
    getAllPages = async (method, cursor) => {
        const result = await method(cursor, LIMIT);
        if(result.data.length !== LIMIT|| !result.cursor) {
            return result.data;
        }
        return result.data.concat(await getAllPages(method, result.cursor));
    },
...
module.exports = {
    ...
    async getFollowsCalendar(userId) {
        ...
        const follows = await getAllPages((cursor, limit) => client.helix.users.getFollows({
                user: userId,
                cursor,
                limit
            })),
